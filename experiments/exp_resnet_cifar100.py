...
# CIFAR-100 superclasses: 0=aquatic mammals, 1=fish
SUPERCLASSES = {
    0: list(range(0, 5)),     # beaver, dolphin, otter, seal, whale
    1: list(range(5, 10)),     # aquarium fish, flatfish, ray, shark, trout
}
...
def run(base_width, seed, device, shuffle=False):
    ...
    train_all = datasets.CIFAR100('/tmp/data', train=True, download=False, transform=transform)
    test_all = datasets.CIFAR100('/tmp/data', train=False, download=False, transform=transform)

    # Filter to superclasses 0 and 1
    def filter_superclass(ds, superclass_labels):
        targets = np.array(ds.targets)
        mask = np.zeros(len(targets), dtype=bool)
        labels = []
        for sc_id, fine_ids in superclass_labels.items():
            for fid in fine_ids:
                m = targets == fid
                mask |= m
                labels.extend([sc_id] * m.sum())
        ds.data = ds.data[mask]
        ds.targets = labels
        return ds
    train_all = filter_superclass(train_all, SUPERCLASSES)
    test_all = filter_superclass(test_all, SUPERCLASSES)

    # Subsample for speed
    rng = np.random.RandomState(42 + seed)
    n_tr = min(4000, len(train_all))
    train_idx = rng.choice(len(train_all), n_tr, replace=False)
    n_te = min(1000, len(test_all))
    test_idx = rng.choice(len(test_all), n_te, replace=False)
    ...
    y_all_te = np.concatenate(y_all_list)
    y_all_tr_list = []
    for _, y in train_loader:
        y_all_tr_list.append(y.numpy())
    y_all_tr = np.concatenate(y_all_tr_list)

    if shuffle:
        np.random.RandomState(42 + seed).shuffle(y_all_tr)

    # Standardize
    y_all_te = (y_all_te - y_all_te.mean()) / y_all_te.std()
    y_all_tr = (y_all_tr - y_all_tr.mean()) / y_all_tr.std()
    ...
    lr0 = Ridge(alpha=1.0).fit(H0, y_all_te)
    err0 = float(np.mean((lr0.predict(H0) - y_all_te)**2))
    y_pred0 = lr0.predict(H0)
    init_r2 = float(1 - np.sum((y_pred0 - y_all_te)**2) / np.sum((y_all_te - y_all_te.mean())**2))
    ...
    lr1 = Ridge(alpha=1.0).fit(H1, y_all_te)
    err1 = float(np.mean((lr1.predict(H1) - y_all_te)**2))
    y_pred1 = lr1.predict(H1)
    post_r2 = float(1 - np.sum((y_pred1 - y_all_te)**2) / np.sum((y_all_te - y_all_te.mean())**2))
    ...
    spec0 = spectral_analysis(H0, y_all_te)
...
if __name__ == '__main__':
    ...
    configs += [(w, s, device, True) for w in BASE_WIDTHS for s in range(3)]  # null: 3 seeds per width
    print(f"Configs: {len(configs)} ({len(BASE_WIDTHS)} widths x {SEEDS} seeds + {len(BASE_WIDTHS)} x 3 null)",
          flush=True)

    results = []
    n_jobs = 1  # sequential execution avoids GPU contention
    ...
    out_path = os.path.join(script_dir, 'key_results', 'resnet_cifar100_results.json')
    ...
    print(f"\n{'='*70}", flush=True)
    print("ResNet-18 on CIFAR-100 (superclass 0 vs 1) — RDEP Calibration", flush=True)

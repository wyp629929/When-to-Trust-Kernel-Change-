"""
Regularized task-aligned diagnostic: eigenvalue shrinkage.
Tests whether shrinking eigenvalue changes improves CV correlation.
"""
...
def run_fold(train_idx, test_idx, seed):
    ...
    proj = evecs0.T @ y_tr
    vi = (proj ** 2) / n_tr
    vi = vi / (np.sum(vi) + 1e-12)

    # Multiple shrinkage levels
    results = {}
    for alpha in [0.0, 0.3, 0.5, 0.7, 0.9]:
        evalsT_shrunk = (1 - alpha) * evalsT + alpha * evals0
        dalign = np.sum(vi * (evalsT_shrunk - evals0)) / (np.sum(vi * evals0) + 1e-12)
        results[f'alpha_{alpha}'] = float(dalign)
...
if __name__ == '__main__':
    ...
    for s in range(5):
        ...
        for fold in range(K):
            ...
            print(f"[{s},{fold}] α=0.0:{r['alpha_0.0']:.4f} α=0.5:{r['alpha_0.5']:.4f} α=0.9:{r['alpha_0.9']:.4f} gain={r['gain']:.4f}")

    for alpha in [0.0, 0.3, 0.5, 0.7, 0.9]:
        vals = [r[f'alpha_{alpha}'] for r in all_folds]
        gains = [r['gain'] for r in all_folds]
        rho, p = spearmanr(vals, gains)
        print(f"α={alpha}: ρ={rho:.4f}, p={p:.4e}")

    out = os.path.expanduser('~/reg_align_v2_results.json')
